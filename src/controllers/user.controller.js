...
const generateAccessAndRefreshTokens = async(userId) => {
   try{
      ...
      console.error("Actual error while generating tokens:", error);
...
const refreshAccessToken = asyncHandler(async (req, res)=>{
    ...
    try {
      const decodedToken = jwtverify(
       ...
       return res 
       .status(200)
       .cooie("accessToken",accessToken, options)
...
const getUserChannelProfile = asyncHandler(async(req, res)=>{
   const {username} = req.params //params for url
   if(!username?.trim()){
      throw new ApiError(400, "username is missing")
   }
   const channel = await User.aggregate([//console log the channel
      {
         $match: {
            username: username?.toLowerCase()
         }
...
]) //we get arrays in aggregate pipeline
   if(!channel?.length){
